    ...
    try
    {
      ...
      else while (instret < n)
      {
        ...
        #define ICACHE_ACCESS(i) { \
          ...
          pc = execute_insn(this, pc, fetch); \  //拿到PC、指令，执行指令
          ic_entry = ic_entry->next; \
          if (i == mmu_t::ICACHE_ENTRIES-1) break; \
          if (unlikely(ic_entry->tag != pc)) break; \   //新生成的pc  不在 取出的ic_entry中
          if (unlikely(instret+1 == n)) break; \        //到达要求的步数
          instret++; \
          state.pc = pc; \
        }
